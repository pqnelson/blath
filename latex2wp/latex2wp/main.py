    ...
    for m in M:
        ...
        if md[0] == '$':
            ...
            if md[0].find('\\begin') != -1:
                count['equation'] += 1
                mb[1] = mb[1] + '\\tag{' + str(count['equation']) + '}'
...
def convert_M(count):
    count[style.theorems['proposition']] += 1
    t = "\n"
    global is_section_open
    if is_section_open:
        t = t + "</section>\n"
    is_section_open = True
    t = t + "<section>\n<p><b>" + str(count[style.theorems['proposition']]) + ".</b> "
    return t

def convert_N(count, label):
    count[style.theorems['proposition']] += 1
    t = "\n"
    global is_section_open
    if is_section_open:
        t = t + "</section>\n"
    is_section_open = True
    t = t + "<section>\n<p><b>" + str(count[style.theorems['proposition']]) + ". "+label+"</b> "
    return t
...
def processtext(t, ref, count, html):
    ...
    while i < len(tcontrol):
        if tcontrol[i].find('{itemize}') != -1:
            ...
            lb = lb.replace('}', '')
            w = w + '</li>\n<li>' + lb
        elif tcontrol[i].find('\\M') != -1:
            w = w + convert_M(count)
        elif tcontrol[i].find('\\N') != -1:
            N_label = tcontrol[i][3:-1]
            if N_label[-1] not in ['.', '?', '!']:
                N_label = N_label + '.'
            ...
            for thm in style.theorems:
                if tcontrol[i] == '\\end{' + thm + '}':
                    w = w + style.thm_style_end.get(thm, '') + style.endthm
                    inthm = ''
